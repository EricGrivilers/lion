...
class CheckAllowURLFopen extends CheckBinaryRequired {
  ...
  function description() {
    return "allow_url_fopen should be enable to allow fething files via HTTP protocol";
  }
  ...
  function run() {
    ...
    if (!$setting) {
      $this->setMessage('<a href="http://php.net/filesystem">allow_url_fopen</a> is disabled. You will not be able to fetch files via HTTP. Demo webinterface will NOT work.');
      return;
    }
