    ...
    */
    ...
    protected function configure() {
        $this
            ...
            // ->addArgument('photos', InputArgument::REQUIRED, 'Photos')
            ->addOption('photos', false, InputOption::VALUE_NONE, 'Photos')
            ->addOption('geo', false, InputOption::VALUE_NONE, 'Geos coords')
            ->addOption('zones', false, InputOption::VALUE_NONE, 'Zones')
            ->addOption('force', false, InputOption::VALUE_NONE, 'Force update')
            ->addOption('full', false, InputOption::VALUE_NONE, 'All options')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        if($input->getOption('full')) {
            $input->setOption('zones', true);
            $input->setOption('photos', true);
            $input->setOption('geo', true);
            $input->setOption('force', true);
        }
        $estateManager->setUp($input->getOption('zones'), $input->getOption('geo'),  $input->getOption('force'));
        $estateManager->import($input->getOption('photos'), $input->getOption('geo'), $input->getOption('force'));
    ...
    /*
        ...
        protected function getActives($t='V') {
            ...
            $existing=array();
            $toProcess=array();
            curl_setopt($rs,CURLOPT_URL,'http://www.esimmo.com/Virtual/'.$this->container->getParameter('evosysId').'/resultats.php?OxySeleOffr='.$t.'&OxySeleBiensParPage=500&OxyPage=1' );
            $xml = curl_exec($rs);
            $estates = new \SimpleXMLElement($xml);
            foreach($estates as $listEstate) {
                $clas=$listEstate->CLAS;
                //echo $clas;
                $clas=str_replace($this->container->getParameter('evosysPrefix')."/","",$clas);
                if($estate= $em->getRepository('CaravaneEstateBundle:Estate')->findOneByReference($this->container->getParameter('evosysPrefix').'/'.$clas)) {
        ...
        protected function import($listEstate,$iType) {
            ...
            if(!$estate= $em->getRepository('CaravaneEstateBundle:Estate')->findOneByReference($this->container->getParameter('evosysPrefix').'/'.$clas)) {
                $date=new \Datetime();
                $estate=new Estate;
                $estate->setReference($this->container->getParameter('evosysPrefix').'/'.$clas);
            ...
            $rs = curl_init();
            curl_setopt($rs,CURLOPT_URL, 'http://www.esimmo.com/Virtual/'.$this->container->getParameter('evosysId').'/offre.php?OxySeleCode='.$listEstate->CODE);
            curl_setopt($rs,CURLOPT_HEADER,0);
            curl_setopt($rs,CURLOPT_RETURNTRANSFER,1);
            curl_setopt($rs,CURLOPT_FOLLOWLOCATION,1);
            $xml = curl_exec($rs);
        ...
        protected function setUp() {
            ...
            $rs = curl_init();
            curl_setopt($rs,CURLOPT_URL,'http://www.esimmo.com/Virtual/'.$this->container->getParameter('evosysId').'/secteurs.php' );
            curl_setopt($rs,CURLOPT_HEADER,0);
            curl_setopt($rs,CURLOPT_RETURNTRANSFER,1);
            curl_setopt($rs,CURLOPT_FOLLOWLOCATION,1);
            $xml = curl_exec($rs);
