...
class admin extends element {
	...
	var $tableFields=array(
		'items'=>array('num','reference','locfr','photo','update'),
	...
	function display() {
		...
		switch($_GET['func']) {
			default:
				$this->listType['type']='items';
				if($_POST['start']) {
					$this->orderLimit=$_POST['start'];
				}
			...
			case 'edit':
				...
				$this->elementName=$_POST['element'];
				$this->dataId=$_POST['dataId'];
				$out=$this->edit();
			break;
			
			case 'addData':
				$this->elementName=$_POST['type'];
				$out=$this->addData();
			break;
			
			case 'saveDatas':
				$this->datas=$_POST['datas'];
				$this->elementName=$_POST['element'];
				$this->dataId=$_POST['dataId'];
				$out=$this->saveDatas();
			break;
			...
			case 'autofill':
				$this->autofillA=array();
				$this->autofillA['type']=$_POST['type'];
				$this->autofillA['queryString']=$_POST['queryString'];
				$out=$this->autofill();
			break;
			
			case 'list':
				$this->listType['type']=$_POST['type'];
				$this->orderLimit=$_POST['start'];
				$out=$this->listAll();
			break;
	...
	*/
	...
	function listAll() {
		...
		if(empty($_POST['orderby'])) {
			$this->orderBy=$this->tableFields[$this->listType['type']][1];	
		}
		else {
			$this->orderBy=	$_POST['orderby'];
		}
		$db->query.=" ORDER BY `".$this->orderBy."` LIMIT ".$this->orderLimit.",15 ";
		$db->setQuery();
		//echo $db->query;
		...
		$out.="<input type='hidden' name='filter' id='filter' value='".$_POST['filter']."' />";
		$out.="<input type='hidden' name='orderby' id='orderby' value='".$_POST['orderby']."' />";
		$out.="<table class='list'>";
		$out.="<thead><tr>";
		foreach($fields as $e=>$f) {
			$out.="<th><a onclick=\"orderBy('".$this->listType['type']."','".($this->orderLimit)."','".$f."')\">".$f."</a></th>";	
		}
	...
	function getFromRef() {
		...
		$db=new DB;
		$db->query="SELECT num FROM items WHERE reference='".$_POST['ref']."'";
		$db->setQuery();
